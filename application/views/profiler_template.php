<style type="text/css">
	...
	.ci-profiler-box { padding: 10px; margin: 0 0 0px 0; max-height: 400px; overflow: auto; color: #fff; font-family: Monaco, 'Lucida Console', 'Courier New', monospace; font-size: 11px !important; }
	...
	<div id="ci-profiler-menu">
		...
		<?php if (isset($sections['queries'])) : ?>
			<a href="#" id="ci-profiler-menu-queries" onclick="ci_profiler_bar.show('ci-profiler-queries', 'ci-profiler-menu-queries'); return false;">
				<span><?php echo is_array($sections['queries']['query']) ? count($sections['queries']['query']) : 0 ?> Queries</span>
				Database
			</a>
		<?php endif; ?>
	...
	<?php if (isset($sections['queries'])) :?>
		<div id="ci-profiler-queries" class="ci-profiler-box" style="display: none">
			...
			<?php if (is_array($sections['queries'])) : ?>
				...
				<?php foreach ($sections['queries']['query'] as $key => $val) : ?>
					<tr><td>
					<p class="hilight">DB : <?php echo $val['db'] ?></p>
					<?php echo $val['sql'] ?></td></tr>
				<?php endforeach; ?>
				</table>

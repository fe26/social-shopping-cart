...
class Fb_ignited {
	...
	function fb_get_me($redirect = false, $script = true) {
		...
		if ($user) {
			try {
				$me = $this->CI->facebook->api('/me?fields=id,name,email,gender');
	...
	function fb_login_url($script = false, $scope = false,$display=false, $redirect = false) {
		/**
		...
		$url = $this->CI->facebook->getLoginUrl($param);
		if ($script == true) {
	...
	function fb_logout_url($next = '') {
		...
		$url = $this->CI->facebook->getLogoutUrl(array(
			'next' => 'http://numpangjualan.com/auth/logout'
				));
		return $url;
	}
